    ...
    if (res.statusCode == 200) {
      ...
      for (i in events) {
        if (events[i].start.timezone == 'Europe/London') {
          ...
          var venue_id = events[i].venue_id
          var venue_search_url = "https://www.eventbriteapi.com/v3/venues/"+ venue_id +"/?token=" + token

         //  function getVenueId(){
         //    request(venue_search_url, function(err, res, body){
         //      if (err) return console.log(err);
         //      if (res.statusCode == 200) {
         //        var data = JSON.parse(body)
